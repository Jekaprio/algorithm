...
type ListNode struct {
	...
	salary   float64   // salary of the person
...
func (l *List) AddItem(name string, age int, gender string, salary float64, err error) {
	if err != nil {
		fmt.Println("incorrect input!!!")
		return
	}
	li := &ListNode{name, age, gender, salary, nil, nil}
	if l.head == nil {
		l.head = li
	} else {
		currentNode := l.tail
...
func (l *List) PrintList() {
	...
	fmt.Print("List: ")
	for list != nil {
		fmt.Printf("Name %s, age %d, gender %s, salary %f -> ", list.name, list.age, list.gender, list.salary)
		list = list.next
	}
	fmt.Println()
}
...
func readPerson() (string, int, string, float64, error) {
	var name string
	var age int
	var gender string
	var salary float64
	...
	return name, age, gender, salary, err
}
